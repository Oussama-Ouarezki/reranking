...
class Qwen06BLFLiT5Cascade:
    """BM25 → Qwen3-0.6B → per-type LF blend → LiT5-LoRA-kaggle-e4 top-20.

    Registered under 'qwen06b_lf_lit5'. Internally uses the LoRA-finetuned
    LiT5 (kaggle epoch 4) instead of the base checkpoint.
    Per-type α: summary=0.99, factoid=0.99, list=0.99, yesno=0.85.
    """

    name = "qwen06b_lf_lit5"

    def __init__(self) -> None:
        self.alpha_by_type = 0.999
        self.fallback      = 0.999
        self.qwen          = registry.get("qwen3_reranker_0_6b")
        self.lit5          = registry.get("lit5_bioasq_lora_kaggle_e2")  # ← swapped
    ...
    def rerank(
        ...
        n = len(merged)
        _log.debug("qwen06b_lf_lit5 (lora_kaggle_e4): qtype=%s alpha=%.3f query=%r",
                   qtype, alpha, query[:60])
