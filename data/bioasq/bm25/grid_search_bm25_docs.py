"""
BM25 grid search over k1 × b at DOCUMENT level on BioASQ.

Corpus  : data/bioasq/processed/corpus.jsonl  (full PubMed abstracts, ~49k docs)
Qrels   : derived from the `documents` field in BioASQ raw data (official
          document-level relevance — NOT snippet-level)
Queries : data/bioasq/processed/queries.jsonl

Compares best params found here against the snippet-level optimum (k1=0.7, b=0.6).

Outputs (all in data/bioasq/bm25/):
  grid_search_results_docs.tsv
  grid_heatmap_hit_at20_docs.png
  grid_heatmap_mean_recall_at20_docs.png
  grid_line_at20_docs.png

Usage:
    python data/bioasq/bm25/grid_search_bm25_docs.py
    python data/bioasq/bm25/grid_search_bm25_docs.py --cutoff 10
"""
...
# snippet-level best for reference
SNIPPET_BEST = {'k1': 0.7, 'b': 0.6}
...
def build_doc_qrels(raw_file, valid_qids: set, valid_pmids: set) -> dict:
    """
    Build qrels from the `documents` field only (official document-level
    relevance in BioASQ). Filters to queries and PMIDs present in the corpus.
    """
...
# ── sparse BM25 index (same as snippet version) ───────────────────────────────

class SparseBM25Index:
    def __init__(self, corpus_texts: list[str], query_texts: list[str]):
        print('  Tokenising and building sparse TF matrices...')
        t0 = time.time()
        ...
        D = len(tok_corpus)
        Q = len(tok_queries)
        print(f'  Vocab: {V:,} | Docs: {D:,} | Queries: {Q:,}')
        ...
        for d_idx, tokens in enumerate(tok_corpus):
            ...
            for t_idx, cnt in counts.items():
                rows.append(d_idx); cols.append(t_idx); data.append(cnt)
                df[t_idx] += 1
        ...
        for q_idx, tokens in enumerate(tok_queries):
            ...
            for t in tokens:
                if t in vocab:
                    counts[vocab[t]] += 1
            for t_idx, cnt in counts.items():
                q_rows.append(q_idx); q_cols.append(t_idx); q_data.append(cnt)
    ...
    def score_batched(self, k1: float, b: float,
        ...
        for s in range(0, Q, batch_size):
            ...
            scores = (batch @ W).toarray().astype(np.float32)   # (B × D)
...
def compute_metrics(top_indices, qids, corpus_ids, qrels):
...
def plot_heatmap(matrix: pd.DataFrame, title: str, label: str, out_path: Path,
    ...
    """mark_cells: list of (k1, b) to outline (e.g. snippet best, anserini default)."""
    ...
    # best doc-level cell (black border)
    best_loc = np.unravel_index(matrix.values.argmax(), matrix.shape)
    ax.add_patch(plt.Rectangle(
        (best_loc[1], best_loc[0]), 1, 1,
        fill=False, edgecolor='#2c3e50', lw=3, label='Best (doc)'
    ))

    # other reference cells
    colors = {'snippet': '#3498db', 'anserini': '#27ae60'}
    if mark_cells:
...
def plot_line_comparison(df: pd.DataFrame, cutoff: int, out_path: Path):
    ...
    for ax, metric, ylabel in zip(
        ...
        # mark snippet-level best and anserini default
        ax.axvline(SNIPPET_BEST['b'], color='#3498db', linestyle=':', lw=1.5,
                   label=f'Snippet best b={SNIPPET_BEST["b"]}')
...
def main():
    ...
    corpus_pmid_set = set(corpus_ids)
    ...
    print('Building document-level qrels from raw BioASQ (documents field)...')
    qrels = build_doc_qrels(RAW_FILE, valid_qids, corpus_pmid_set)
    ...
    print(f'\n{"─"*55}')
    print(f'  Snippet corpus avgdl : ~35 tokens  (for reference)')
    print(f'  Document corpus avgdl: {index.avgdl:.0f} tokens')
    print(f'  → Higher b will over-penalise long docs; expect optimal b to shift lower')
    print(f'{"─"*55}\n')

    # ── grid search ───────────────────────────────────────────────────────────
    total = len(K1_VALUES) * len(B_VALUES)
    print(f'Grid search: {len(K1_VALUES)} k1 × {len(B_VALUES)} b = {total} combos  '
          f'(cutoff @{cutoff})\n')
    ...
    snip = df[(df['k1'] == SNIPPET_BEST['k1']) & (df['b'] == SNIPPET_BEST['b'])].iloc[0]
    ...
    print(f'  {"Snippet-level best":<30} {snip[f"pct_hit@{cutoff}"]:<12.2f} {snip[f"mean_recall@{cutoff}"]:.2f}%')
    print(f'    k1={SNIPPET_BEST["k1"]}, b={SNIPPET_BEST["b"]}')
    ...
    # ── save ──────────────────────────────────────────────────────────────────
    tsv_path = OUT_DIR / 'grid_search_results_docs.tsv'
    ...
    ref_cells = [
        (f'Snippet best (k1={SNIPPET_BEST["k1"]},b={SNIPPET_BEST["b"]})',
         SNIPPET_BEST['k1'], SNIPPET_BEST['b'], '#3498db'),
    ...
    plot_heatmap(hit_m,
                 ...
                 f'% hit@{cutoff}',
                 OUT_DIR / f'grid_heatmap_hit_at{cutoff}_docs.png',
                 mark_cells=ref_cells)
    plot_heatmap(recall_m,
                 f'BM25 Document Level — Mean Per-Query Recall@{cutoff}',
                 f'Mean Recall@{cutoff} (%)',
                 OUT_DIR / f'grid_heatmap_mean_recall_at{cutoff}_docs.png',
                 mark_cells=ref_cells)
    plot_line_comparison(df, cutoff, OUT_DIR / f'grid_line_at{cutoff}_docs.png')
