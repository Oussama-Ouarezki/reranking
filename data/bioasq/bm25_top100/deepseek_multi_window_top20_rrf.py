"""
Multi-window listwise reranking with RRF fusion on BioASQ top-35 BM25 docs.

Strategy: rerank 4 overlapping windows of 20 docs each, then fuse with
Reciprocal Rank Fusion (RRF) to produce the best possible top-20.

Windows (step=5, size=20):
  Window 1: BM25 positions  [0 : 20]
  Window 2: BM25 positions  [5 : 25]
  Window 3: BM25 positions [10 : 30]
  Window 4: BM25 positions [15 : 35]

Each window is reranked independently with DeepSeek-Chat (title trick, 512 tok).
Rankings are merged via RRF (K=60): score = sum(1 / (K + rank)) across windows.
The final permutation is sorted by descending RRF score.
...
Writes:
  data/bioasq/bm25_top100/deepseek_multi_window_top20_rrf.jsonl

Usage:
    export DEEPSEEK_API_KEY="sk-..."
    cd /home/oussama/Desktop/reranking_project
    /home/oussama/miniconda3/envs/pyml/bin/python \\
        data/bioasq/bm25_top100/deepseek_multi_window_top20_rrf.py --n-queries 1
"""
...
OUT_FILE = BASE / "data/bioasq/bm25_top100/deepseek_multi_window_top20_rrf.jsonl"

WINDOW_SIZE = 20
STEP        = 5
N_WINDOWS   = 4          # windows start at 0, 5, 10, 15
MAX_TOKENS  = 512
RRF_K       = 60
MODEL       = "deepseek-chat"

OFFSETS = [i * STEP for i in range(N_WINDOWS)]   # [0, 5, 10, 15]
N_DOCS  = OFFSETS[-1] + WINDOW_SIZE              # 35 BM25 docs needed
...
def rerank_window(
    ...
    docs = [corpus.get(did, {"title": "", "text": ""}) for did in doc_ids]
    messages = [
        {"role": "system", "content": SYSTEM_PROMPT},
        {"role": "user",   "content": build_user_prompt(query, docs, bm25_scores)},
    ]
    try:
        raw     = call_api(client, model, messages)
        ranking = parse_ranking(raw, len(doc_ids))
        return [doc_ids[i - 1] for i in ranking]
    except Exception:
        return doc_ids


def rrf_fuse(window_rankings: list[list[str]], k: int = RRF_K) -> list[str]:
    scores: dict[str, float] = defaultdict(float)
    for ranked in window_rankings:
        for rank, docid in enumerate(ranked):
            scores[docid] += 1.0 / (k + rank + 1)
    return sorted(scores, key=lambda d: scores[d], reverse=True)


def multi_window_rerank(
    client, model: str, query: str,
    all_doc_ids: list[str], all_bm25_scores: list[float],
    corpus: dict[str, dict],
) -> list[str]:
    window_rankings: list[list[str]] = []
    for offset in OFFSETS:
        w_ids    = all_doc_ids[offset: offset + WINDOW_SIZE]
        w_scores = all_bm25_scores[offset: offset + WINDOW_SIZE]
        ranked   = rerank_window(client, model, query, w_ids, w_scores, corpus)
        window_rankings.append(ranked)
        time.sleep(0.1)
    return rrf_fuse(window_rankings)
...
def main() -> None:
    ...
    print(f"Config: {N_WINDOWS} windows | offsets={OFFSETS} | size={WINDOW_SIZE} | "
          f"step={STEP} | RRF_K={RRF_K} | max_tokens={MAX_TOKENS} | model={args.model}")
    print(f"Covers BM25 positions 0-{N_DOCS - 1} ({N_DOCS} docs per query)")
    ...
    needed = {h["docid"] for r in records for h in r["top100"][:N_DOCS]}
    ...
    with OUT_FILE.open("a") as out:
        for rec in tqdm(records, desc="Reranking"):
            top_hits    = rec["top100"][:N_DOCS]
            doc_ids     = [h["docid"] for h in top_hits]
            bm25_scores = [h["score"] for h in top_hits]
            bm25_order  = [h["docid"] for h in rec["top100"][:50]]  # full top-50 for eval
            try:
                permutation = multi_window_rerank(
