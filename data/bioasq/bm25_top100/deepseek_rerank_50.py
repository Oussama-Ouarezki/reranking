"""
Pass top-50 BM25 PubMed abstracts per query to DeepSeek for listwise ranking.
...
Writes:
  data/bioasq/bm25_top100/deepseek_reranked_50.jsonl

Output per query:
  {
    "qid":         "...",
    "bm25_order":  ["docid1", ..., "docid50"],   # original BM25 order
...
Usage:
    ...
    /home/oussama/miniconda3/envs/pyml/bin/python \\
        data/bioasq/bm25_top100/deepseek_rerank_50.py --n-queries 10
    /home/oussama/miniconda3/envs/pyml/bin/python \\
        data/bioasq/bm25_top100/deepseek_rerank_50.py          # all 2000
"""
...
OUT_FILE = BASE / "data/bioasq/bm25_top100/deepseek_reranked_50.jsonl"
...
def main() -> None:
    ...
    with OUT_FILE.open("a") as out:
        for rec in tqdm(records, desc="Reranking"):
            top50       = rec["top100"][:TOP_N]
            top50_ids   = [h["docid"] for h in top50]
            bm25_scores = [h["score"] for h in top50]
            docs        = [corpus.get(did, {"title": "", "text": ""}) for did in top50_ids]
            messages    = [
            ...
            try:
                ...
                permutation = [top50_ids[i - 1] for i in ranking]
                n_ok       += 1
            except Exception as e:
                print(f"\n  [FAIL] {rec['qid']}: {e}")
                permutation = top50_ids
                n_fail     += 1

            out.write(json.dumps({
                "qid":         rec["qid"],
                "bm25_order":  top50_ids,
