"""
Self-consistency reranking with RRF fusion on BioASQ top-50 BM25 docs.

Runs the same sliding-window reranker (prompt_2, top_n=50, w=20, s=10)
N_RUNS times on each query, then fuses all permutations with Reciprocal
Rank Fusion (RRF, K=60) to reduce non-determinism noise.

Each individual run uses temperature=0 but the model is non-deterministic
across calls, so multiple runs vote out random swaps and stabilize the top
positions.
...
Usage:
    ...
    /home/oussama/miniconda3/envs/pyml/bin/python \\
        data/bioasq/bm25_top100/deepseek_self_consistency_rrf.py --n-queries 1
"""
...
OUT_FILE = BASE / "data/bioasq/bm25_top100/deepseek_self_consistency_rrf.jsonl"

TOP_N       = 50
WINDOW_SIZE = 20
STEP        = 10
MAX_TOKENS  = 512
N_RUNS      = 3       # number of independent reranking passes per query
RRF_K       = 60
MODEL       = "deepseek-chat"
...
def main() -> None:
    ...
    parser.add_argument("--n-runs",    type=int, default=N_RUNS,
                        help=f"Number of independent runs to fuse (default {N_RUNS})")
    ...
    print(f"Config: top-{TOP_N} docs | window={WINDOW_SIZE} | step={STEP} | "
          f"max_tokens={MAX_TOKENS} | n_runs={args.n_runs} | RRF_K={RRF_K} | model={args.model}")
        ...
        for rec in tqdm(records, desc="Reranking"):
            ...
            try:
                ...
                for run_i in range(args.n_runs):
                    ...
                    runs.append(perm)
                    if run_i < args.n_runs - 1:
                        time.sleep(0.5)
                permutation = rrf_fuse(runs)
                n_ok += 1
