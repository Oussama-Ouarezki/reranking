"""
Two-stage reranking: sliding window + top-20 refinement pass.

Stage 1 — Sliding window (same as prompt_2):
  top-50 BM25 candidates, window=20, step=10, 4 passes bottom-up.

Stage 2 — Refinement pass:
  Take the top-20 from Stage 1 and rerank all 20 in a single API call
  so the model has full global context over the final candidates.
...
Writes:
  data/bioasq/bm25_top100/deepseek_sliding_refined_top20.jsonl
...
Usage:
    ...
    /home/oussama/miniconda3/envs/pyml/bin/python \\
        data/bioasq/bm25_top100/deepseek_sliding_refined_top20.py --n-queries 1
    /home/oussama/miniconda3/envs/pyml/bin/python \\
        data/bioasq/bm25_top100/deepseek_sliding_refined_top20.py
"""
...
OUT_FILE = BASE / "data/bioasq/bm25_top100/deepseek_sliding_refined_top20.jsonl"

TOP_N       = 50
WINDOW_SIZE = 20
STEP        = 10
REFINE_K    = 20   # final refinement window size
MAX_TOKENS  = 512
MODEL       = "deepseek-chat"
...
def refinement_pass(client, model: str, query: str,
                    doc_ids: list, bm25_scores: list,
                    corpus: dict) -> list:
    """Single-pass rerank of top-K docs with full global context."""
    w_docs = [corpus.get(did, {"title": "", "text": ""}) for did in doc_ids]
    messages = [
        {"role": "system", "content": SYSTEM_PROMPT},
        {"role": "user",   "content": build_user_prompt(query, w_docs, bm25_scores)},
    ]
    try:
        raw     = call_api(client, model, messages)
...
def main() -> None:
    ...
    n_windows = (TOP_N - WINDOW_SIZE) // STEP + 1
    print(f"Stage 1: top-{TOP_N} | window={WINDOW_SIZE} | step={STEP} | ~{n_windows} passes")
    print(f"Stage 2: refinement pass on top-{REFINE_K}")
    print(f"Total API calls/query: ~{n_windows + 1} | {MAX_TOKENS} tok/doc | model={args.model}")
    ...
    with OUT_FILE.open("a") as out:
        for rec in tqdm(records, desc="Reranking"):
            ...
            try:
                # Stage 1: sliding window over top-50
                stage1 = sliding_window_rerank(
                    client, args.model, rec["query"],
                    doc_ids, bm25_scores, corpus)

                # Stage 2: refinement pass on top-20 from stage 1
                top20_ids    = stage1[:REFINE_K]
                top20_scores = [bm25_scores[doc_ids.index(d)] for d in top20_ids]
                time.sleep(0.1)
                refined_top20 = refinement_pass(
                    client, args.model, rec["query"],
                    top20_ids, top20_scores, corpus)

                # Final permutation: refined top-20 + rest from stage 1 unchanged
                permutation = refined_top20 + stage1[REFINE_K:]
                n_ok += 1
