...
Variant: prompt_2_norm — same as prompt_2 but uses normalized BM25 score (0–1)
in the prompt instead of raw BM25 score, so the model sees a clearer relative signal.
...
Algorithm (RankGPT-style, bottom-up):
  For N=50 docs, window=20, step=10:
    ...
    Pass 4: rerank positions [0-20]

Each document is truncated to 512 tokens using tiktoken.
Uses deepseek-chat (temperature=0, JSON mode).

Reads:
  data/bioasq/bm25_top100/bm25_top100_ids.jsonl      (must contain norm_score field)
  data/bioasq/pubmed_full/full/corpus_full_processed.jsonl

Writes:
  data/bioasq/bm25_top100/deepseek_sliding_reranked_prompt_2_norm.jsonl
...
Usage:
    ...
    /home/oussama/miniconda3/envs/pyml/bin/python \\
        data/bioasq/bm25_top100/deepseek_sliding_window_prompt_2_norm.py --n-queries 10
    /home/oussama/miniconda3/envs/pyml/bin/python \\
        data/bioasq/bm25_top100/deepseek_sliding_window_prompt_2_norm.py
"""
...
OUT_FILE = BASE / "data/bioasq/bm25_top100/deepseek_sliding_reranked_prompt_2_norm.jsonl"
...
SYSTEM_PROMPT = (
    ...
    "The BM25 score (0=lowest, 1=highest in this pool) is a meaningful signal — "
    "top-ranked BM25 documents are usually correct. "
...
def build_user_prompt(query: str, docs: list[dict], norm_scores: list[float]) -> str:
    n = len(docs)
    ...
    for i, (doc, norm) in enumerate(zip(docs, norm_scores), start=1):
        lines.append(f"[{i}] BM25={norm:.3f}")
...
def sliding_window_rerank(
    ...
    doc_ids: list[str], norm_scores: list[float],
    corpus: dict[str, dict],
) -> list[str]:
    ranked = list(zip(doc_ids, norm_scores))
    n = len(ranked)
    ...
    while True:
        ...
        w_norms    = [s for _, s in window]
        w_docs     = [corpus.get(did, {"title": "", "text": ""}) for did in w_ids]
        messages   = [
            {"role": "system", "content": SYSTEM_PROMPT},
            {"role": "user",   "content": build_user_prompt(query, w_docs, w_norms)},
        ]

        try:
            raw     = call_api(client, model, messages)
            ranking = parse_ranking(raw, len(window))
            reranked_window = [(w_ids[i - 1], w_norms[i - 1]) for i in ranking]
        except Exception:
...
def main() -> None:
    ...
    print(f"Config: top-{args.top_n} docs | window={args.window} | step={args.step} | "
          f"~{n_windows} windows/query | {MAX_TOKENS} tokens/doc | model={args.model} | norm_score=True")
    ...
    with OUT_FILE.open("a") as out:
        for rec in tqdm(records, desc="Reranking"):
            ...
            norm_scores = [h["norm_score"] for h in top_hits]

            try:
                permutation = sliding_window_rerank(
                    client, args.model, rec["query"],
                    doc_ids, norm_scores, corpus,
                )
                n_ok += 1
