"""
Sliding-window listwise reranking with DeepSeek-Reasoner (R1) on BioASQ top-50 BM25 docs.
Same config as prompt_2 but uses deepseek-reasoner instead of deepseek-chat.

Note: deepseek-reasoner does not support response_format or temperature=0.
      Ranking is parsed from free-text output using fallback strategies.

Config: top_n=50, window=20, step=10, max_tokens=512.
...
Writes:
  data/bioasq/bm25_top100/deepseek_sliding_reranked_prompt_2_reasoner.jsonl

Usage:
    export DEEPSEEK_API_KEY="sk-..."
    cd /home/oussama/Desktop/reranking_project
    /home/oussama/miniconda3/envs/pyml/bin/python \\
        data/bioasq/bm25_top100/deepseek_sliding_window_prompt_2_reasoner.py --n-queries 10
"""
...
OUT_FILE = BASE / "data/bioasq/bm25_top100/deepseek_sliding_reranked_prompt_2_reasoner.jsonl"
...
SYSTEM_PROMPT = (
    ...
    "Respond ONLY with JSON. No explanation after the JSON.\n"
...
def main() -> None:
    ...
    print(f"Config: top-{TOP_N} docs | window={WINDOW_SIZE} | step={STEP} | "
          f"max_tokens={MAX_TOKENS} | model={args.model}")
