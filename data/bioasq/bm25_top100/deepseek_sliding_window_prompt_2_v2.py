...
Variant: prompt_v2 — passages truncated to 512 tokens, window=10, step=5.
...
Writes:
  data/bioasq/bm25_top100/deepseek_sliding_reranked_prompt_v2_top20_w10_s5.jsonl

Output per query:
  {
    "qid":         "...",
    "bm25_order":  ["docid1", ..., "docid20"],
...
Usage:
    ...
    /home/oussama/miniconda3/envs/pyml/bin/python \\
        data/bioasq/bm25_top100/deepseek_sliding_window_top20_w10_s5.py --n-queries 10
    /home/oussama/miniconda3/envs/pyml/bin/python \\
        data/bioasq/bm25_top100/deepseek_sliding_window_top20_w10_s5.py          # all queries
"""
...
OUT_FILE = BASE / "data/bioasq/bm25_top100/deepseek_sliding_reranked_prompt_v2.jsonl"

TOP_N       = 50    # ← was 50
WINDOW_SIZE = 20    # ← was 20
STEP        = 10     # ← was 10
MAX_TOKENS  = 512
MODEL       = "deepseek-chat"
...
# ── System prompt — APEER V2 preference-aligned ───────────────────────────
# ── System prompt — V3 (topical match primary, evidence quality demoted) ──────
SYSTEM_PROMPT = (
...
def sliding_window_rerank(
    ...
    """
    Bottom-up sliding window reranking over top-20 docs.

    For N=20, window=10, step=5:
      Pass 1: indices [10:20]  → rerank last 10
      Pass 2: indices [5:15]   → rerank middle 10
      Pass 3: indices [0:10]   → rerank first 10
    """
...
def main() -> None:
    ...
    parser.add_argument("--top-n",     type=int, default=TOP_N,
                        help="Number of BM25 candidates to rerank (default 20)")
    ...
    # How many windows per query: e.g. top_n=20, window=10, step=5 → 3 passes
    ...
    # ── Load corpus — only top-N docs needed ──────────────────────────────────
    ...
    with OUT_FILE.open("a") as out:
        for rec in tqdm(records, desc="Reranking"):
            top_hits    = rec["top100"][:args.top_n]   # ← only take top-20
