"""
Listwise reranking with DeepSeek-Chat on BioASQ top-20 BM25 docs.

No sliding window — 20 passages fit in one API call (window = N).
Uses Prompt V3: topical match primary, evidence quality demoted to tiebreaker.
...
Usage:
    ...
    /home/oussama/miniconda3/envs/pyml/bin/python \\
        data/bioasq/bm25_top100/deepseek_top20_512_prompt_2.py --n-queries 10
    /home/oussama/miniconda3/envs/pyml/bin/python \\
        data/bioasq/bm25_top100/deepseek_top20_512_prompt_2.py          # all queries
"""
...
OUT_FILE = BASE / "data/bioasq/bm25_top100/deepseek_top20_512_reranked_prompt_2.jsonl"
...
def truncate_doc(title: str, text: str) -> str:
...
def main() -> None:
    ...
    print(f"Config: top-{TOP_N} docs | single pass (no sliding window) | "
          f"{MAX_TOKENS} tokens/doc | model={args.model}")
    ...
    with OUT_FILE.open("a") as out:
        for rec in tqdm(records, desc="Reranking"):
            ...
            try:
                permutation = rerank_single_pass(
                    client, rec["query"], doc_ids, bm25_scores, corpus,
                )
                n_ok += 1
