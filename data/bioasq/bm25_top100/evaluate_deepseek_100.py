...
Writes:
  data/bioasq/bm25_top100/images/evaluation_bm25_vs_deepseek100.png
...
DS100_FILE   = BASE / "data/bioasq/bm25_top100/deepseek_reranked_100.jsonl"
IMG_DIR     = BASE / "data/bioasq/bm25_top100/images"

TOP_N     = 100
NDCG_KS   = [1, 5, 10, 20, 50, 100]
PREC_KS   = [1, 5, 10, 20, 50, 100]
RECALL_KS = [1, 5, 10, 20, 50, 100]
MRR_KS    = [1, 5, 10, 20, 50, 100]
MAP_KS    = [1, 5, 10, 20, 50, 100]
...
def _draw_panel(ax: plt.Axes, metrics: list[str], bm25: dict[str, float],
    ...
    bars_d = ax.bar(x + w / 2, ds_vals,   w, label="DeepSeek-Reasoner",
...
def plot(bm25: dict[str, float], ds: dict[str, float], n_queries: int) -> None:
    ...
    fig.suptitle(
        f"BM25 vs DeepSeek-Reasoner Reranking — BioASQ  ({n_queries} queries, top-100)",
        fontsize=13, y=1.01,
    )
    ...
    out = IMG_DIR / "evaluation_bm25_vs_deepseek100.png"
    fig.tight_layout()
...
def main() -> None:
    ...
    print(f"Loading DeepSeek reranked results from {DS100_FILE.name} …")
    ds_results = load_reranked(DS100_FILE)
