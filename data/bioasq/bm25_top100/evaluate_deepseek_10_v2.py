"""
Evaluate BM25 (top-20) vs DeepSeek-Chat reranked (top-20) on BioASQ.

Metrics: nDCG@1/5/10/20, P@1/5/10/20, R@1/5/10/20, MRR@1/5/10/20, MAP@1/5/10/20

Reads:
  data/bioasq/processed/qrels.tsv
  data/bioasq/bm25_top100/bm25_top100_ids.jsonl
  data/bioasq/bm25_top100/deepseek_chat_reranked_20_512.jsonl

Writes:
  data/bioasq/bm25_top100/images/evaluation_bm25_vs_deepseek_chat20_512.png

Usage:
    cd /home/oussama/Desktop/reranking_project
    /home/oussama/miniconda3/envs/pyml/bin/python \\
        data/bioasq/bm25_top100/evaluate_deepseek_20.py
"""
...
DS20_512_FILE  = BASE / "data/bioasq/bm25_top100/deepseek_sliding_reranked_prompt_10_v2.jsonl"
...
def _draw_panel(ax: plt.Axes, metrics: list[str], bm25: dict[str, float],
    ...
    bars_b = ax.bar(x - w / 2, bm25_vals, w, label="BM25 top-20",
                    color="#4C72B0", alpha=0.88)
    bars_d = ax.bar(x + w / 2, ds_vals,   w, label="DeepSeek-Chat (top-20)",
                    color="#C44E52", alpha=0.88)

    for bars, color, vals in [(bars_b, "#4C72B0", bm25_vals),
                               (bars_d, "#C44E52", ds_vals)]:
...
def plot(bm25: dict[str, float], ds: dict[str, float], n_queries: int) -> None:
    ...
    fig.suptitle(
        f"BM25 vs DeepSeek-Chat Reranking — BioASQ  ({n_queries} queries, top-20, 512 tokens/doc)",
        fontsize=13, y=1.01,
    )
    ...
    out = IMG_DIR / "evaluation_bm25_vs_deepseek_chat20_512.png"
    fig.tight_layout()
...
def main() -> None:
    ...
    print(f"Loading DeepSeek-Chat reranked results from {DS20_512_FILE.name} …")
    ds_results = load_reranked(DS20_512_FILE)
    ...
    header = f"  {'Metric':<12}  {'BM25':>8}  {'DeepSeek-Chat':>14}  {'Δ':>8}"
    print(f"\n{header}")
    print("  " + "─" * 50)
    for m in metrics:
        delta = ds_scores[m] - bm25_scores[m]
        sign  = "+" if delta >= 0 else ""
        print(f"  {m:<12}  {bm25_scores[m]:>7.2f}%  {ds_scores[m]:>13.2f}%  "
              f"{sign}{delta:>6.2f}%")
