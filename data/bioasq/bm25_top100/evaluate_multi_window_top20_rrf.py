"""
Evaluate BM25 vs DeepSeek multi-window RRF reranking on BioASQ.
...
Writes:
  data/bioasq/bm25_top100/images/evaluation_multi_window_top20_rrf.png

Usage:
    cd /home/oussama/Desktop/reranking_project
    /home/oussama/miniconda3/envs/pyml/bin/python \\
        data/bioasq/bm25_top100/evaluate_multi_window_top20_rrf.py
"""
...
DS_FILE   = BASE / "data/bioasq/bm25_top100/deepseek_multi_window_top20_rrf.jsonl"
...
def plot(bm25, ds, n_queries):
    ...
    fig.suptitle(
        f"BM25 vs DeepSeek Multi-Window RRF (4×w20 step5, 512 tok) — BioASQ ({n_queries} queries)",
        fontsize=13, y=1.01,
    )
    for ax, (metrics, title), show_legend in zip(axes, panels, [True, False, False, False, False]):
        ...
        bars_d = ax.bar(x + w/2, dv, w, label="Multi-window RRF (4×w20)", color="#C44E52", alpha=0.88)
        for bars, color, vals in [(bars_b, "#4C72B0", bv), (bars_d, "#C44E52", dv)]:
    ...
    out = IMG_DIR / "evaluation_multi_window_top20_rrf.png"
    fig.tight_layout()
...
def main():
    ...
    print(f"Loading multi-window RRF results from {DS_FILE.name} …")
    ...
    print(f"\n4 windows [0-20, 5-25, 10-30, 15-35] | RRF K=60 | max_tokens=512 | n_queries={len(qids)}")
    print(f"  {'Metric':<12}  {'BM25':>8}  {'Multi-RRF':>10}  {'Δ':>8}")
    print("  " + "─" * 46)
    for m in metrics:
