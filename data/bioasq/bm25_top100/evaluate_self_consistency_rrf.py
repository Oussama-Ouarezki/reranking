"""
Evaluate BM25 vs DeepSeek self-consistency RRF reranking on BioASQ.

Reads:
  data/bioasq/processed/qrels.tsv
  data/bioasq/bm25_top100/bm25_top100_ids.jsonl
  data/bioasq/bm25_top100/deepseek_self_consistency_rrf.jsonl

Writes:
  data/bioasq/bm25_top100/images/evaluation_self_consistency_rrf.png

Usage:
    cd /home/oussama/Desktop/reranking_project
    /home/oussama/miniconda3/envs/pyml/bin/python \\
        data/bioasq/bm25_top100/evaluate_self_consistency_rrf.py
"""
...
DS_FILE   = BASE / "data/bioasq/bm25_top100/deepseek_self_consistency_rrf.jsonl"
...
# ── Plot ──────────────────────────────────────────────────────────────────────
def plot(bm25, ds, n_queries, n_runs):
    panels = [
    ...
    fig.suptitle(
        f"BM25 vs DeepSeek Self-Consistency ×{n_runs} RRF (top-50, w=20, s=10) — BioASQ ({n_queries} queries)",
        fontsize=13, y=1.01,
    )
    for ax, (metrics, title), show_legend in zip(axes, panels, [True, False, False, False, False]):
        ...
        bars_d = ax.bar(x + w/2, dv, w, label=f"Self-consistency ×{n_runs} RRF", color="#8172B2", alpha=0.88)
        for bars, color, vals in [(bars_b, "#4C72B0", bv), (bars_d, "#8172B2", dv)]:
    ...
    out = IMG_DIR / "evaluation_self_consistency_rrf.png"
    fig.tight_layout()
...
def main():
    ...
    print(f"Loading self-consistency RRF results from {DS_FILE.name} …")
    ds_results, n_runs = load_reranked(DS_FILE)
    ...
    print(f"\ntop_n=50 | window=20 | step=10 | n_runs={n_runs} | RRF_K=60 | n_queries={len(qids)}")
    print(f"  {'Metric':<12}  {'BM25':>8}  {'SC-RRF':>10}  {'Δ':>8}")
    print("  " + "─" * 46)
    for m in metrics:
    ...
    print("\nGenerating plot …")
    plot(bm25_scores, ds_scores, len(qids), n_runs)
    print("Done.")
