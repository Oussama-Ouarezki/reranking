"""
Evaluate BM25 (top-20) vs DeepSeek single-pass reranked (top-20) on BioASQ.

Reads:
  data/bioasq/processed/qrels.tsv
  data/bioasq/bm25_top100/bm25_top100_ids.jsonl
  data/bioasq/bm25_top100/deepseek_single_pass_top20_reranked.jsonl

Writes:
  data/bioasq/bm25_top100/images/evaluation_single_pass_top20.png

Usage:
    cd /home/oussama/Desktop/reranking_project
    /home/oussama/miniconda3/envs/pyml/bin/python \\
        data/bioasq/bm25_top100/evaluate_single_pass_top20.py
"""
...
DS_FILE   = BASE / "data/bioasq/bm25_top100/deepseek_single_pass_top20_reranked.jsonl"
...
def plot(bm25, ds, n_queries):
    ...
    fig.suptitle(
        f"BM25 vs DeepSeek Single-Pass (top-20, 512 tok) — BioASQ ({n_queries} queries)",
        fontsize=13, y=1.01,
    )
    for ax, (metrics, title), show_legend in zip(axes, panels, [True, False, False, False, False]):
        ...
        bars_d = ax.bar(x + w/2, dv, w, label="DeepSeek single-pass top-20", color="#55A868", alpha=0.88)
    ...
    out = IMG_DIR / "evaluation_single_pass_top20.png"
    fig.tight_layout()
...
def main():
    ...
    print(f"\nprompt_2 (title trick) | top_n=20 | single pass | max_tokens=512 | n_queries={len(qids)}")
