...
Reads:
  ...
  data/bioasq/bm25_top100/deepseek_sliding_reranked.jsonl

Writes:
  data/bioasq/bm25_top100/images/evaluation_bm25_vs_sliding_window.png
...
DS_SW_FILE   = BASE / "data/bioasq/bm25_top100/deepseek_sliding_reranked.jsonl"
...
def _draw_panel(ax: plt.Axes, metrics: list[str], bm25: dict[str, float],
    ...
    bars_d = ax.bar(x + w / 2, ds_vals,   w, label="DeepSeek-Chat Sliding (w=20/s=10)",
...
def plot(bm25: dict[str, float], ds: dict[str, float], n_queries: int) -> None:
    ...
    fig.suptitle(
        f"BM25 vs DeepSeek-Chat Sliding Window — BioASQ  ({n_queries} queries, top-50, w=20/s=10)",
        fontsize=13, y=1.01,
    )
    ...
    out = IMG_DIR / "evaluation_bm25_vs_sliding_window.png"
    fig.tight_layout()
