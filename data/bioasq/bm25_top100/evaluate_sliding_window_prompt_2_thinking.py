"""
Evaluate BM25 (top-50) vs DeepSeek-Chat + thinking reranked (top-50) on BioASQ.
...
Writes:
  data/bioasq/bm25_top100/images/evaluation_sliding_window_prompt_2_thinking.png

Usage:
    cd /home/oussama/Desktop/reranking_project
    /home/oussama/miniconda3/envs/pyml/bin/python \\
        data/bioasq/bm25_top100/evaluate_sliding_window_prompt_2_thinking.py
"""
...
DS_FILE   = BASE / "data/bioasq/bm25_top100/deepseek_sliding_reranked_prompt_2_thinking.jsonl"
...
def plot(bm25, ds, n_queries):
    ...
    fig.suptitle(
        f"BM25 vs DeepSeek-Chat + Thinking (top-50, w=20, s=10, 512 tok) — BioASQ ({n_queries} queries)",
        fontsize=13, y=1.01,
    )
    for ax, (metrics, title), show_legend in zip(axes, panels, [True, False, False, False, False]):
        ...
        bars_d = ax.bar(x + w/2, dv, w, label="DeepSeek + thinking",     color="#DD8452", alpha=0.88)
        for bars, color, vals in [(bars_b, "#4C72B0", bv), (bars_d, "#DD8452", dv)]:
    ...
    out = IMG_DIR / "evaluation_sliding_window_prompt_2_thinking.png"
    fig.tight_layout()
...
def main():
    ...
    print(f"Loading DeepSeek+thinking results from {DS_FILE.name} …")
    ...
    print(f"\ntop_n=50 | window=20 | step=10 | thinking=ON | max_tokens=512 | n_queries={len(qids)}")
    print(f"  {'Metric':<12}  {'BM25':>8}  {'Thinking':>10}  {'Δ':>8}")
    print("  " + "─" * 46)
    for m in metrics:
