"""
Pass top-20 BM25 PubMed abstracts per query to DeepSeek-Chat for listwise ranking.
...
Usage:
    ...
    /home/oussama/miniconda3/envs/pyml/bin/python \\
        data/bioasq/bm25_top100/deepseek_rerank_20.py --n-queries 10
    /home/oussama/miniconda3/envs/pyml/bin/python \\
        data/bioasq/bm25_top100/deepseek_rerank_20.py          # all 2000
"""
...
OUT_FILE = BASE / "data/bioasq/bm25_top100/prompt engineering/deepseek_chat_reranked_20_512.jsonl"
...
SYSTEM_PROMPT = (
    ...
    "Each entry has a TITLE and an abstract. The TITLE is a strong relevance signal — "
    "weight it heavily alongside the BM25 score and abstract content when judging relevance. "
...
def build_user_prompt(query: str, docs: list[dict], bm25_scores: list[float]) -> str:
    ...
    for i, (doc, score) in enumerate(zip(docs, bm25_scores), start=1):
        title    = doc["title"].rstrip(".") if doc["title"] else "No title"
        abstract = _truncate_text(doc["text"])
    ...
    lines.append(
        ...
        f"The TITLE is a strong relevance signal — weight it heavily alongside the BM25 score "
        f"and the abstract content. "
...
def parse_ranking(raw: str, n: int) -> list[int]:
    ...
    Strategy 2: Last bracket array with >= n//2 items.
    Strategy 3: Scrape all integers from text in order.
    Fallback:   Identity (BM25 order unchanged).
    Always returns a complete list of length n.
    """
    # Strategy 1 — JSON object (primary path with JSON mode enabled)
    try:
        obj = json.loads(raw)
