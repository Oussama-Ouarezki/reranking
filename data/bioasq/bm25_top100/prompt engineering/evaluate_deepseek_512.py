...
Usage:
    ...
    /home/oussama/miniconda3/envs/pyml/bin/python \\
        data/bioasq/bm25_top100/evaluate_deepseek_512.py
"""
...
DS_FILE   = BASE / "data/bioasq/bm25_top100/prompt engineering/deepseek_reranked_512.jsonl"
...
def load_reranked(path: Path) -> dict[str, list[str]]:
    ...
    with path.open() as f:
        for line in f:
            r = json.loads(line)
            ranked = r.get("reranked") or r.get("permutation")
            if ranked:
...
def _draw_panel(ax: plt.Axes, metrics: list[str], bm25: dict[str, float],
    ...
    bars_d = ax.bar(x + w / 2, ds_vals,   w, label="DeepSeek-Chat (512 tok/passage)",
