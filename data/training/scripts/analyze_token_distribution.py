"""
Token length distribution analysis for the truncated BioASQ training corpus.

Reads from  : data/training/truncated/corpus.jsonl
Writes to   : data/training/images/
...
Usage:
    python data/training/scripts/analyze_token_distribution.py
"""
...
DOC_DIR   = BASE / 'data' / 'training' / 'truncated'
OUT_DIR   = BASE / 'data' / 'training' / 'images'
...
def plot_histogram(lengths: list[int], out_path: Path):
    ...
    ax.set_title('Token Length Distribution — Truncated BioASQ Training Corpus', fontsize=14, pad=12)
...
def plot_cumulative(lengths: list[int], out_path: Path):
    ...
    ax.set_title('Cumulative Distribution of Token Lengths — Truncated BioASQ Training Corpus',
                 fontsize=14, pad=12)
...
def plot_boxplot(lengths: list[int], out_path: Path):
    ...
    ax.set_title('Token Length Boxplot — Truncated BioASQ Training Corpus', fontsize=14, pad=12)
    ax.set_xlabel('Token count per document', fontsize=12)
    ax.set_yticks([])
