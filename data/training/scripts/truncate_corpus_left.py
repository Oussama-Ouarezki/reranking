"""
Truncate BioASQ training corpus documents to a fixed token limit, keeping the
TAIL (right side) of each document — i.e. conclusions and closing sentences.

Reads from  : data/bioasq/processed/corpus.jsonl
Writes to   : data/training/truncated_left/corpus.jsonl

Usage:
    python data/training/scripts/truncate_corpus_left.py
    python data/training/scripts/truncate_corpus_left.py --limit 256
    python data/training/scripts/truncate_corpus_left.py --limit 280 --tokenizer castorini/LiT5-Distill-base
"""
...
OUT_DIR   = BASE / 'data' / 'training' / 'truncated_left'
...
def truncate_text_left(text: str, tokenizer, limit: int) -> str:
    ids = tokenizer.encode(text, add_special_tokens=False)
    if len(ids) <= limit:
        return text
    # keep the last `limit` tokens (tail / conclusion)
    return tokenizer.decode(ids[-limit:], skip_special_tokens=True)


def main():
    parser = argparse.ArgumentParser(
        description='Truncate corpus texts to a token limit, keeping the tail.'
    )
    parser.add_argument(
    ...
    with SRC_PATH.open(encoding='utf-8') as src, \
        ...
        for line in src:
            ...
            doc['text']  = truncate_text_left(doc['text'], tokenizer, args.limit)
    ...
    print(f'Token limit         : {args.limit} T5 tokens  (tail kept)')
