"""Exp 6: LiT5-Distill (checkpoints/LiT5-Distill-base) listwise rerank on the
top-20 docs by Qwen3-0.6B probability.

Top-20 fits a single LiT5 window (WINDOW_SIZE=20), so one decode per query.
Tail (rank 21..50 from Qwen) is pinned below the LiT5-ranked head.
"""
...
OUT_TREC   = BASE / "qwen3_0.6b/results/run_qwen_lit5_top20.tsv"
OUT_JSON   = BASE / "qwen3_0.6b/results/metrics_qwen_lit5_top20.json"
OUT_SCORES = BASE / "qwen3_0.6b/data/lit5_scores_qwen.jsonl"
...
def main() -> None:
    ...
    for qid in tqdm(qids, unit="q"):
        ...
        # Score: descending integer ranks (head above tail)
        ...
        for t_idx, docid in enumerate(tail_ids):
            score = float(len(tail_ids) - t_idx)
            # Pin tail strictly below head
            score = -1.0 - t_idx * 1e-3
            run.append(ScoredDoc(qid, docid, score))
            per_qid.append({"rank": n_head + t_idx + 1, "docid": docid, "score": score})

        jsonl_records.append({"qid": qid, "type": qtypes.get(qid), "ranked": per_qid})

    metrics = report("Exp 6: Qwen + LiT5 top-20", run, qrels, qtypes)
    write_trec(OUT_TREC, run, tag="qwen06b_lit5_top20")
