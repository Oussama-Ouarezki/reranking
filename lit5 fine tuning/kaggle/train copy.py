...
Experiment variant (Kaggle): r=8 LoRA on [q,k,v,o] — same rank/LR/epochs as
the local run (which uses [q,v]). The only variable is the module set, so
this is a clean A/B for whether k,o projections help at this gentleness.

Configuration:
  - LoRA: r=8, alpha=16, dropout=0.05, target_modules=[q,k,v,o]
  ...
  - Epochs: 2 (save + validate after each — keep best by nDCG@1)
...
# Goal: gentle nudge over a strong zero-shot baseline. Variant of the local
# run — same rank/LR/epochs, but adds k,o projections to test whether they
# help on top of the q,v-only configuration.
LR = 5e-6
EPOCHS = 2              # 2 epochs × 2400 windows
...
# Use a fixed subset of N test queries so per-epoch eval is fast AND
# comparable across epochs. The same N queries are used every time — no
# random subsampling between runs. Sorted-by-qid for cross-run determinism.
EVAL_FIXED_N = 5
