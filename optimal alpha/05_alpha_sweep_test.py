"""LF α sweep on the BioASQ Task13BGoldenEnriched TEST set (340 queries).

Same α grid as the training-set sweeps for direct comparison.

Reads:  qwen3_0.6b/data/qwen06b_scores_test.jsonl
        data/bioasq/raw/Task13BGoldenEnriched/qrels.tsv
Writes: optimal alpha/results/alpha_sweep_test.tsv
        optimal alpha/results/alpha_best_test.json
        optimal alpha/plots/alpha_bar_clusters_test.png
"""
...
OUT_TSV  = BASE / "optimal alpha/results/alpha_sweep_test.tsv"
OUT_JSON = BASE / "optimal alpha/results/alpha_best_test.json"
OUT_PLOT = BASE / "optimal alpha/plots/alpha_bar_clusters_test.png"
...
def main() -> None:
    ...
    ax.set_ylabel("score")
    ax.set_title(f"LF α sweep — Qwen3-0.6B on {len(qids)} BioASQ TEST queries (Task13BGoldenEnriched)\n"
