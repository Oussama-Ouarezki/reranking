"""H@50 + RRF fusion optimisation for nDCG@1, @5, @10.

Mirrors 08_h50_optimize.py but uses Reciprocal Rank Fusion instead of linear:

    score(d) = 1 / (k + rank_qwen(d)) + 1 / (k + rank_bm25(d))

Two stages:

Stage 1 — Best k (always fuse, no gate)
    Find k* per scope per metric.

Stage 2 — Best (k, τ) with H@50 gate
    fuse with RRF when H@50(query) > τ, else keep pure Qwen.
    Find joint optimum (k, τ) per scope per metric.

Reads:  qwen4b_uncertainty/data/qwen_scores.jsonl
        data/bioasq/processed/qrels.tsv
Writes: qwen4b_uncertainty/data/h50_optim_rrf.tsv
        qwen4b_uncertainty/plots/h50_rrf_k_only.png
        qwen4b_uncertainty/plots/h50_rrf_k_tau_global.png
        qwen4b_uncertainty/plots/h50_rrf_k_tau_by_type.png
"""
...
OUT_TSV = BASE / "qwen4b_uncertainty/data/h50_optim_rrf.tsv"
...
# RRF k grid — standard k=60 in the literature; sweep covers small-k (more weight
# on top ranks) to large-k (flatter contribution).
K_GRID = [1, 2, 5, 10, 15, 20, 30, 40, 50, 60, 80, 100, 150, 200, 300]
...
def rank_array(x: np.ndarray) -> np.ndarray:
    """1-based ranks (rank 1 = highest score)."""
...
def main() -> None:
    ...
    qwen_rank: dict[str, np.ndarray] = {}
    bm25_rank: dict[str, np.ndarray] = {}
    ...
    for r in rows:
        ...
        qwen_arr[qid] = q
        qwen_rank[qid] = rank_array(q)
        bm25_rank[qid] = rank_array(b)
    ...
    # ── Stage 1 — sweep k only (always RRF, no gate) ───────────────────────
    print("\n=== Stage 1: RRF k only (no gate) ===")
    rows_s1: list[dict] = []
    for k in tqdm(K_GRID, desc="k sweep"):
        run: list[ScoredDoc] = []
        for qid in qids:
            s = 1.0 / (k + qwen_rank[qid]) + 1.0 / (k + bm25_rank[qid])
            for d, sv in zip(docids[qid], s):
                run.append(ScoredDoc(qid, d, float(sv)))
        for sc, qs in scopes:
            m = evaluate(run, qrels, qs)
            rows_s1.append({"stage": 1, "scope": sc, "k": int(k),
                            "tau": np.nan, **m})

    # ── Stage 2 — joint sweep (k, τ) with H@50 gate ────────────────────────
    print("\n=== Stage 2: RRF k + H@50 gate (fuse when H@50 > τ) ===")
    rows_s2: list[dict] = []
    for k in tqdm(K_GRID, desc="k×τ sweep"):
        for tau in TAUS:
            run: list[ScoredDoc] = []
            for qid in qids:
                if H50[qid] > tau:
    ...
    print("\n" + "=" * 90)
    print("STAGE 1 — Best k (always RRF, no gate)")
    print("=" * 90)
    print(f"{'scope':<8}  {'metric':<7}  {'k*':>4}  {'value':>8}  "
          f"{'baseline':>8}  {'Δ':>7}")
    for sc in ["global"] + TYPES:
        for m in METRIC_NAMES:
            b = best_stage1(sc, m)
            bv = base[sc][m]
            print(f"{sc:<8}  {m:<7}  {int(b['k']):>4}  {b[m]:>8.4f}  "
                  f"{bv:>8.4f}  {b[m]-bv:>+7.4f}")

    print("\n" + "=" * 90)
    print("STAGE 2 — Best (k, τ) with H@50 gate (fuse when H@50 > τ)")
    print("=" * 90)
    print(f"{'scope':<8}  {'metric':<7}  {'k*':>4}  {'τ*':>7}  {'value':>8}  "
          f"{'baseline':>8}  {'Δ_qwen':>8}  {'Δ_stage1':>10}  {'%fused':>6}")
    for sc in ["global"] + TYPES:
        for m in METRIC_NAMES:
            b = best_stage2(sc, m)
            bv = base[sc][m]
            s1 = best_stage1(sc, m)[m]
            scope_qids = type_qids[sc] if sc in TYPES else set(qids)
            n_fused = sum(1 for q in scope_qids if H50[q] > b["tau"])
            pct = 100.0 * n_fused / len(scope_qids)
            print(f"{sc:<8}  {m:<7}  {int(b['k']):>4}  {b['tau']:>7.4f}  "
                  f"{b[m]:>8.4f}  {bv:>8.4f}  {b[m]-bv:>+8.4f}  "
                  f"{b[m]-s1:>+10.4f}  {pct:>5.1f}%")

    # ── Plot 1: Stage 1 — k curves per scope, three metrics ────────────────
    fig, axes = plt.subplots(1, 3, figsize=(20, 5), sharex=True)
    for ax, m in zip(axes, METRIC_NAMES):
        for sc in ["global"] + TYPES:
            sub = df[(df["stage"] == 1) & (df["scope"] == sc)].sort_values("k")
            ax.plot(sub["k"], sub[m], marker="o", ms=3, lw=1.8, label=sc)
            ax.axhline(base[sc][m], color="gray", ls=":", lw=0.7, alpha=0.5)
        ax.set_xlabel("RRF k")
        ax.set_ylabel(m)
        ax.set_xscale("log")
        ax.set_title(f"Stage 1 — RRF k only — {m}", fontweight="bold")
        ax.legend(fontsize=8, loc="best")
    fig.tight_layout()
    p = PLOTS / "h50_rrf_k_only.png"
    ...
    for col, t in enumerate(TYPES):
        for row, m in enumerate(METRIC_NAMES):
            ...
            sns.heatmap(pivot, ax=ax, cmap="viridis",
                        cbar_kws={"label": m}, xticklabels=8, yticklabels=1)
            b = best_stage2(t, m)
            ax.set_title(f"{t} — {m}\nk={int(b['k'])} τ={b['tau']:.4f}  "
            ...
            if col == 0:
                ...
                ax.set_ylabel("")
    fig.suptitle("Stage 2 — RRF k × τ heatmaps per query type",
                 fontsize=14, fontweight="bold")
    fig.tight_layout()
    p = PLOTS / "h50_rrf_k_tau_by_type.png"
    fig.savefig(p, dpi=160); plt.close(fig)
    print(f"  → {p}")
