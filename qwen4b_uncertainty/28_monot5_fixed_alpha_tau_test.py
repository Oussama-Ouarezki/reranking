"""Fixed-alpha + H@20 entropy gate tau sweep — monoT5, Task13BGoldenEnriched test set.

Alpha is fixed to the per-type best from script 27.
Sweeps tau over [0, 1] (101 steps) with H@20 normalised entropy gate.

    if H@20(query) > tau:
        score(d) = alpha_type * monot5_norm(d) + (1-alpha_type) * bm25_norm(d)
    else:
        score(d) = monot5_prob(d)
...
Writes: qwen4b_uncertainty/data/monot5_fixed_alpha_tau_test.tsv
        qwen4b_uncertainty/data/monot5_fixed_alpha_tau_best_test.json
        qwen4b_uncertainty/plots/monot5_fixed_alpha_tau_test.png
"""
...
ALPHA_F  = BASE / "qwen4b_uncertainty/data/monot5_alpha_best_test.json"
OUT_TSV  = BASE / "qwen4b_uncertainty/data/monot5_fixed_alpha_tau_test.tsv"
OUT_JSON = BASE / "qwen4b_uncertainty/data/monot5_fixed_alpha_tau_best_test.json"
...
def main() -> None:
    ...
    print(f"\nH@20 range: {min(h_vals):.3f} .. {max(h_vals):.3f}  "
          f"(mean={np.mean(h_vals):.3f})")
    for t in TYPES:
        ht = [H20[q] for q in type_qids[t]]
        print(f"  {t:<8}  n={len(ht):3d}  mean={np.mean(ht):.3f}  "
    ...
    # Stage-1 reference (always fuse with fixed alpha per scope)
    s1: dict[str, dict] = {}
    for sc, qs in scopes:
        ...
        for qid in qids:
            ...
            for d, sv in zip(docids[qid], scores):
                run.append(ScoredDoc(qid, d, float(sv)))
        s1[sc] = evaluate(run, qrels, qs)

    print("\nBaselines:")
    print(f"  {'scope':<8}  {'alpha':>6}  "
          + "  ".join(f"{'monot5_'+m:>14}" for m in METRIC_NAMES)
          + "  "
          + "  ".join(f"{'s1_'+m:>11}" for m in METRIC_NAMES))
    for sc, _ in scopes:
        mv = "  ".join(f"{base[sc][m]:>14.4f}" for m in METRIC_NAMES)
        sv = "  ".join(f"{s1[sc][m]:>11.4f}" for m in METRIC_NAMES)
        print(f"  {sc:<8}  {fixed_alpha[sc]:>6.3f}  {mv}  {sv}")
    ...
    for tau in tqdm(TAUS, desc="tau"):
        for sc, qs in scopes:
            ...
            n_fused = sum(1 for q in qs if H20[q] > tau)
            sweep_rows.append({
                "scope": sc, "alpha": fixed_alpha[sc], "tau": float(tau),
                "n_fused": n_fused,
                "pct_fused": round(100.0 * n_fused / len(qs), 1),
                **m,
            })
    ...
    print("\n" + "=" * 115)
    print("BEST TAU PER SCOPE — monoT5 fixed alpha + H@20 gate")
    print("=" * 115)
    print(f"  {'scope':<8}  {'alpha':>6}  {'tau*':>6}  {'%fused':>7}  "
          + "  ".join(f"{m:>8}" for m in METRIC_NAMES)
          + "  " + "  ".join(f"{'Δmonot5_'+m:>13}" for m in METRIC_NAMES)
    ...
    for sc, qs in scopes:
        ...
        results[sc] = {
            ...
            "baseline_monot5":   {m: float(bv[m]) for m in METRIC_NAMES},
            "baseline_s1":       {m: float(sv[m]) for m in METRIC_NAMES},
            "delta_vs_monot5":   {m: float(b[m] - bv[m]) for m in METRIC_NAMES},
    ...
    for ax, (sc, qs) in zip(axes, scopes):
        ...
        ax.set_title(
            ...
            f"Dotted=pure monoT5  Dashed=always-fuse  Red=τ*",
            fontweight="bold", fontsize=9,
        )
        ax.legend(fontsize=7, loc="lower left")

    fig.suptitle(
        "monoT5 fixed-alpha entropy gate: tau sweep per query type\n"
        "Task13BGoldenEnriched test set — H@20 normalised entropy",
        fontsize=11, fontweight="bold",
    )
    fig.tight_layout()
    p = PLOTS / "monot5_fixed_alpha_tau_test.png"
