"""Cache monoT5-base-msmarco-100k scores for the BioASQ train set (500 queries).

Reads:  qwen4b_uncertainty/data/bm25_top50.jsonl
Writes: qwen4b_uncertainty/data/monot5_scores.jsonl

Resumable: skips qids already present.
"""
...
IN   = BASE / "qwen4b_uncertainty/data/bm25_top2000.jsonl"
OUT  = BASE / "qwen4b_uncertainty/data/monot5_scores_2000.jsonl"
...
class Scorer:
    ...
    def _score_batch(self, queries, passages):
        ...
        decoder_input = torch.zeros((len(inputs), 1), dtype=torch.long,
                                    device=self.device)
        with torch.no_grad():
            logits = self.model(
...
def main():
    queries = [json.loads(l) for l in IN.open()]
    ...
    with OUT.open("a") as f:
        for q in tqdm(todo, unit="q"):
            ...
            f.write(json.dumps({"qid": q["qid"], "type": q["type"],
                                "scores": scores}) + "\n")
            f.flush()
    print(f"done -> {OUT}")
